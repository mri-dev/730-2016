<?	class simple extends Controller{
		function __construct(){
			parent::__construct();
			parent::$pageTitle = 'Simple - Kártyás fizetés';
		...
		****************************************/
		function back(){
			require_once( SimpleAPI::getConfigFile() );
			require_once( SimpleAPI::getPaymentFile() );
			...
			if($backref->checkResponse()){
				...
				if( $order[alerted_user_about_pay] == 0 ){
					...
					if( strtoupper($order[priceCode]) != 'HUF' ){
						...
						$msg .= 'Simple transaction ID: '.$backStatus['PAYREFNO'];
						...
						$msg .= 'Simple tranzakció: '.$backStatus['PAYREFNO'].'<br>';
		...
		****************************************/
		function irn(){
			require_once( SimpleAPI::getConfigFile() );
			require_once( SimpleAPI::getPaymentExtraFile() );
