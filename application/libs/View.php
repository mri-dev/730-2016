<?	class View {
		...
		function addStyle($style, $after = '', $source = true){
			if($source){
				return '<link rel="stylesheet" type="text/css" href="' . STYLE . $style .'.css?v='.VERSION.( (DEVMODE) ? '&t='.microtime() : '' ).'" '.$after.'/>'."\n\r";
			}else{
				return '<link rel="stylesheet" type="text/css" href="' . SSTYLE . $style .'.css?v='.VERSION.( (DEVMODE) ? '&t='.microtime() : '' ).'" '.$after.'/>'."\n\r";
			}
		}
		...
		function addJS($file, $type = false, $source = true){
			if($source){
				...
				return '<script type="text/javascript" src="'.$wt.'?v='.VERSION.( (DEVMODE) ? '&t='.microtime() : '' ).'"></script>'."\n\r";
			}else{
